...
def train(model, criterion, optimizer, data, metric, mtype, ctype, contrastive=None, encoder = True, classification = True):
    ...
    for batch_idx in range(num_batches):
        ...
        # print(data[batch_idx]["audio"].shape,data[batch_idx]["emo"].shape)
        ...
        if (len(label_tensor.shape)<=1):
            label_tensor = score_tensor.unsqueeze(0)
        # prepare data for in1put to model
        ...
        # print("target::::",model_target1)
        model_target2 = model_target[num:][r2]
        model_label1 =  model_label[:num][r1]
        model_label2=  model_label[num:][r2]
        ...
        if encoder:
            ...
            loss = contrastive(model_label1, model_label2, model_output1, model_output2)
            # print(loss)
            ...
            # print(model_target1.size(),model_input1.size())
            ...
            if classification:
                ...
                t1 = model_target1.view(model_target1.shape[0],model_target1.shape[2])
                t2 = model_target2.view(model_target2.shape[0],model_target2.shape[2])
            # print("output1",model_output1.size(),"\noutput2",model_output2.size(),"\ntarget1",t1.size(),"\ntarget2",t2.size())
