...
def eval_model(model, criterion, data, metric, mtype, ctype, extra_outs = 0):
    ...
    for batch_idx in range(num_batches):
        ...
        # print(batch_idx)
        pitch_tensor = torch.Tensor(data[batch_idx]['audio'])
        score_tensor = torch.Tensor(data[batch_idx]['emo'])
        label_tensor = torch.Tensor(data[batch_idx]['label'])
        if len(pitch_tensor.shape)!=3:
        ...
        model_label = label_tensor.clone()
        ...
        # print(batch_idx,model_input.size())
        if mtype == 'lstm':
            model.init_hidden(mini_batch_size)
        try:
            if type(model) is PCConvNetContrastive2:
...
def eval_model_preds(model, criterion, data, metric, mtype, ctype, extra_outs = 0, latent = False):
    ...
    for batch_idx in range(num_batches):
        # extract pitch tensor and score for the batch
        pitch_tensor = data[batch_idx]['audio']
        score_tensor = data[batch_idx]['emo']
        label_tensor = torch.Tensor(data[batch_idx]['label'])
        score_tensor = torch.unsqueeze(score_tensor, 1)
        # prepare data for input to model
        model_input = pitch_tensor.clone()
...
def eval_acc_contrastive(model, criterion, data, metric, mtype, ctype, extra_outs = 0, classification = True, criterion_CE = None, preds = False):
        ...
        for batch_idx in range(1, num_batches - 1, 2):
            ...
            score_tensor = torch.Tensor(data[batch_idx]['emo'])
            label_tensor = torch.Tensor(data[batch_idx]["label"])
            pitch_tensor2 = torch.Tensor(data[batch_idx-1]['audio'])
            score_tensor2 = torch.Tensor(data[batch_idx-1]['emo'])
            label_tensor2 = torch.Tensor(data[batch_idx-1]["label"])
            # prepare data for input to model
            if (len(pitch_tensor.shape)<=2):
                pitch_tensor=pitch_tensor.unsqueeze(0)
            if (len(pitch_tensor2.shape) <= 2):
                pitch_tensor2=pitch_tensor2.unsqueeze(0)
            if (len(label_tensor.shape)<=2):
                label_tensor=label_tensor.unsqueeze(0)
            if (len(label_tensor2.shape) <= 2):
                label_tensor2=label_tensor2.unsqueeze(0)
            if (len(score_tensor.shape)<=1):
                score_tensor=score_tensor.unsqueeze(0)
            if (len(score_tensor2.shape)<=1):
                score_tensor2=score_tensor2.unsqueeze(0)

            # print("eval_shape:::",pitch_tensor2.shape,score_tensor2.shape)
            model_input = pitch_tensor.clone()
            model_target = score_tensor.clone()
            model_input2 = pitch_tensor2.clone()
            model_target2 = score_tensor2.clone()
            model_label = label_tensor.clone()
            model_label2 = label_tensor2.clone()
            if ctype == 1:
                #model_input = model_input.long()
                model_target = model_target.long()
            # convert to cuda tensors if cuda available
            if torch.cuda.is_available():
                ...
                model_label = label_tensor.cuda()
                model_label2 = label_tensor2.cuda()
            # wrap all tensors in pytorch Variable
            model_input = Variable(model_input)
            model_target = Variable(model_target)
            ...
            model_label = Variable(model_label)
            model_label2 = Variable(model_label2)
            # compute forward pass for the network
            mini_batch_size = model_input.size(0)
            if mtype == 'lstm':
                model.init_hidden(mini_batch_size)
            ...
            loss = criterion(model_label, model_label2, conv_out1, conv_out2)
            # loss = criterion(model_target, model_target2, conv_out1, conv_out2)
            if criterion_CE:
                ce_loss = criterion_CE(model_output1, model_target) + criterion_CE(model_output2, model_target2.squeeze(dim=1))
            ...
            if ctype == 0:
                ...
                # print(torch.argmax(torch.cat((model_target, model_target2), dim=0),dim=1))
                pred = torch.argmax(pred, dim=1)
                correct_s = torch.sum(torch.eq(pred, (torch.argmax(torch.cat((model_target, model_target2), dim=0),dim=1))))
                # correct_s = torch.sum(torch.eq(pred, (torch.cat((model_target, model_target2), dim=0).squeeze())).float())
                correct += correct_s
                total += pred.shape[0]
